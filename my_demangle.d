...
template Demangle(T)
{
	enum Demangle = demangleT(T.mangleof);
}
...
string demangleT(string name)
{
	ParseContext P;
	P.name = name;

	if( name.length >= 3 && (name[0..2]=="_D" && isdigit(name[2])) )
	{
		P.ni = 2;
		//goto Lnot;
	}else{
...
//	try
//	{
		auto result = P.parseQualifiedName();
		result = P.parseType(result);
		while(P.ni < name.length){
				result ~= " . " ~ P.parseType(P.parseQualifiedName());
		}
...
/+	static string[2][] table =
	...
	static const table = baseTypes ~ arrayTypes ~ pointerTypes ~ storageClasses;

	foreach (i, name; table)
	{
	//	writefln("%s", name[0]);
		string r = demangleT(name[0]);
		assert(r == name[1],
